...
void BaseProtocol::prepareSend(BasePacket* pack) {
	...
	if (disableEncryption && pack->doEncryption()) {
...
void BaseProtocol::encrypt(Packet* pack, bool crc) const {
	debug() << "encrypting packet: " << *pack;

	char* pData = pack->getBuffer();
	int nLength = pack->size();

	uint32 nCrcSeed = crcSeed;
